        ...
        )
{
    ...
    struct my_blob_info* pbi = NULL;

    SG_ERR_CHECK(  SG_rbtree__find(pCtx, pData->prb_blob_info, szHidBlob, &b_already, (void**) &pbi)  );
    if (b_already && pbi)
    {
        if (ppsz_objectid)
        {
            if (pbi->psz_objectid)
            {
                SG_ERR_CHECK(  SG_STRDUP(pCtx, pbi->psz_objectid, ppsz_objectid)  );
            }
            else
            {
                *ppsz_objectid = NULL;
        ...
        if (ppsz_hid_vcdiff_reference)
        {
            ...
            {
                SG_ERR_CHECK(  SG_STRDUP(pCtx, pbi->psz_hid_vcdiff_reference, ppsz_hid_vcdiff_reference)  );
            }
            else
            {
        ...
        if (p_blob_encoding)
        {
            *p_blob_encoding = pbi->blob_encoding;
        }
        if (p_len_encoded)
        {
            *p_len_encoded = pbi->len_encoded;
        }
        if (p_len_full)
        {
            *p_len_full = pbi->len_full;
        }
        if (p_filenumber)
        {
            *p_filenumber = pbi->filenumber;
        }
        if (p_offset)
        {
            *p_offset = pbi->offset;
	...
	SG_ERR_CHECK(  sg_sqlite__prepare(pCtx, pData->psql,&pStmt,
									  "SELECT encoding, len_encoded, len_full, hid_vcdiff, filenumber, offset, objectid FROM directory WHERE hid = ?")  );
    ...
    if (SQLITE_ROW == rc)
    {
        ...
        psz_objectid = (const char *)sqlite3_column_text(pStmt,6);

        if (ppsz_objectid)
        {
            if (psz_objectid)
            {
                SG_ERR_CHECK(  SG_STRDUP(pCtx, psz_objectid, ppsz_objectid)  );
            }
            else
            {
                *ppsz_objectid = NULL;
...
void sg_blob_fs3_handle_store__free(SG_context* pCtx, sg_blob_fs3_handle_store* pbh)
{
    ...
    if (pbh->pRHH_ComputeOnStore)
	{
		...
		SG_ERR_IGNORE(  sg_repo__fs3__hash__abort(pCtx, pbh->ptx->pData->pRepo, &pbh->pRHH_ComputeOnStore)  );
	}
    ...
    )
{
    ...
    while (b)
    {
        SG_ERR_CHECK(  sg_fs3__get_filenumber_path__sz(pCtx, ptx->pData, psz_filenumber, &pPath_file)  );
    ...
    )
{
    ...
    while (1)
    {
        ...
        SG_ERR_CHECK(  sg_fs3__get_filenumber_path__sz(pCtx, ptx->pData, buf_file, &pPath_file)  );
        ...
        pPath_lock = NULL;
        SG_ERR_CHECK(  sg_fs3__lock(pCtx, ptx->pData, buf_file, &pPath_lock)  );
    ...
    )
{
    ...
    SG_ERR_CHECK(  sg_fs3__find_a_place__already_locked(pCtx, ptx, space, &filenumber)  );

    if (!filenumber)
    {
        SG_ERR_CHECK(  sg_fs3__find_a_place__new_lock(pCtx, ptx, space, &filenumber)  );
    }
    ...
    )
{
    ...
    SG_ERR_CHECK(  SG_rbtree__find(pCtx, ptx->prb_file_handles, buf, &b_already, (void**) &pFile)  );
    if (pFile)
    {
        pbh->pFileBlob = pFile;
    }
    ...
    {
        SG_ERR_CHECK(  sg_fs3__get_filenumber_path__sz(pCtx, ptx->pData, buf, &pPathnameFile)  );
        SG_ERR_CHECK(  SG_file__open__pathname(pCtx, pPathnameFile ,SG_FILE_WRONLY|SG_FILE_OPEN_EXISTING,0644,&pbh->pFileBlob)  );
        SG_ERR_CHECK(  SG_rbtree__add__with_assoc(pCtx, ptx->prb_file_handles, buf, pbh->pFileBlob)  );
    }
    ...
    )
{
    ...
    {
		SG_ERR_CHECK(  sg_repo__fs3__hash__begin(pCtx, ptx->pData->pRepo, &pbh->pRHH_ComputeOnStore)  );
    }
    else
    {
    ...
    SG_ERR_CHECK(  sg_fs3__find_a_place(pCtx, ptx, space_needed, &pbh->filenumber)  );

    SG_ERR_CHECK(  sg_fs3__open_file_for_writing(pCtx, ptx, pbh)  );
    ...
    )
{
    ...
    if (pbh->pRHH_ComputeOnStore)
    {
		SG_ERR_CHECK(  sg_repo__fs3__hash__chunk(pCtx, pbh->ptx->pData->pRepo, pbh->pRHH_ComputeOnStore, len_chunk, p_chunk)  );
    }
    ...
    )
{
    ...
    if (pbh->pRHH_ComputeOnStore)
    {
		...
		SG_ERR_CHECK(  sg_repo__fs3__hash__end(pCtx, pbh->ptx->pData->pRepo, &pbh->pRHH_ComputeOnStore, &szHidBlob)  );
    ...
    if (pbh->psz_objectid)
    {
        SG_ERR_CHECK(  SG_STRDUP(pCtx, pbh->psz_objectid, (char**) &pbi->psz_objectid)  );
    }
...
void sg_fs3__free_pbi(SG_context* pCtx, void * pVoid_pbi)
{
    ...
    SG_NULLFREE(pCtx, pbi->psz_objectid);
...
static void sg_blob_fs3__open_seekreader(
	SG_context * pCtx,
    my_instance_data* pData,
	...
	)
{
    ...
    SG_ERR_CHECK(  sg_fs3__fetch_blob_into_tempfile(pCtx, pData, szHidBlob, &pPath_tempfile)  );
	...
	)
{
	...
	{
		SG_RETRY_THINGIE(
			my_fetch_info(pCtx, pData, szHidBlob, &filenumber, &offset, &blob_encoding_stored, &len_encoded_stored, &len_full_stored, &psz_objectid, &psz_hid_vcdiff_reference_stored)
			);
	}
	else
	{
		SG_ERR_CHECK(  my_fetch_info(pCtx, pData, szHidBlob, &filenumber, &offset, &blob_encoding_stored, &len_encoded_stored, &len_full_stored, &psz_objectid, &psz_hid_vcdiff_reference_stored)  );
	}
    ...
    pbh->psz_hid_vcdiff_reference_stored_freeme = psz_hid_vcdiff_reference_stored;
    pbh->psz_objectid_freeme = psz_objectid;
...
static void _sg_blob_handle__close(SG_context * pCtx, sg_blob_fs3_handle_fetch * pbh)
{
    ...
    SG_NULLFREE(pCtx, pbh->psz_objectid_freeme);
    ...
    )
{
    ...
    SG_ERR_CHECK(  sg_blob_fs3__open_seekreader(pCtx,
                pData,
                psz_hid_vcdiff_reference,
                &pbh->psr_reference,
                &pbh->pPath_tempfile
    ...
    SG_ERR_CHECK(  SG_readstream__alloc(pCtx,
                pbh->pbh_delta,
                (SG_stream__func__read*) sg_blob_fs3__fetch_blob__chunk,
                NULL,
                &pbh->pstrm_delta)  );
    ...
    )
{
            ...
            )
    {
        ...
        else if (SG_BLOBENCODING__VCDIFF == pbh->blob_encoding_stored)
        {
            ...
            SG_ERR_CHECK(  sg_blob_fs3__setup_undeltify(pCtx, pData, psz_hid_blob, pbh, pbh->psz_hid_vcdiff_reference_stored_freeme)  );
        }
        else
        {
            /* should never happen */
    ...
    if (ppsz_objectid)
    {
        if (pbh->psz_objectid_freeme)
        {
            SG_ERR_CHECK(  SG_STRDUP(pCtx, pbh->psz_objectid_freeme, ppsz_objectid)  );
        }
        else
        {
            *ppsz_objectid = NULL;
...
void sg_blob_fs3__get_blob_stats(
    SG_context * pCtx,
    sqlite3* psql,
...
void sg_repo__fs3__open_repo_instance(SG_context * pCtx, SG_repo * pRepo)
{
    ...
    SG_ERR_CHECK(  SG_rbtree__alloc(pCtx, &pData->prb_blob_info)  );
...
fail:
    ...
    if(pData!=NULL)
    {
		SG_RBTREE_NULLFREE(pCtx, pData->prb_blob_info);
...
#define sg_MY_CLOSET_DIR_PREFIX				"fs3_"
#define sg_MY_CLOSET_DIR_LENGTH_RANDOM		32											// how many hex digits we want in the name
#define sg_MY_CLOSET_DIR_SUFFIX				".fs3"
#define sg_MY_CLOSET_DIR_BUFFER_LENGTH		(4 + sg_MY_CLOSET_DIR_LENGTH_RANDOM + 4 + 1)	// "fs3_" + <hex_digits> + ".fs3" + NULL
...
void sg_repo__fs3__close_repo_instance(SG_context * pCtx, SG_repo * pRepo)
{
    ...
    SG_ERR_IGNORE(  sg_blob_fs3_handle_store__free(pCtx, pData->pBlobStoreHandle)  );
	SG_NULLFREE(pCtx, pData);

	SG_ERR_REPLACE(SG_ERR_SQLITE(SQLITE_BUSY), SG_ERR_DB_BUSY);
}

void sg_repo__fs3__fetch_dagnode(SG_context * pCtx, SG_repo * pRepo, const char* pszidHidChangeset, SG_dagnode ** ppNewDagnode)	// must match FN_
{
	my_instance_data * pData = NULL;
...
void sg_repo__fs3__list_dags(SG_context * pCtx, SG_repo* pRepo, SG_uint32* piCount, SG_uint32** paDagNums)
{
	my_instance_data * pData = NULL;
	pData = (my_instance_data *)pRepo->p_vtable_instance_data;
...
void sg_repo__fs3__fetch_dag_leaves(SG_context * pCtx, SG_repo * pRepo, SG_uint32 iDagNum, SG_rbtree ** ppIdsetReturned)	// must match FN_
{
	my_instance_data * pData = NULL;
	pData = (my_instance_data *)pRepo->p_vtable_instance_data;
...
void sg_repo__fs3__fetch_dagnode_children(SG_context * pCtx, SG_repo * pRepo, SG_uint32 iDagNum, const char* pszDagnodeID, SG_rbtree ** ppIdsetReturned)	// must match FN_
{
	my_instance_data * pData = NULL;
	pData = (my_instance_data *)pRepo->p_vtable_instance_data;
...
void sg_repo__fs3__check_dag_consistency(SG_context * pCtx, SG_repo * pRepo, SG_uint32 iDagNum)	// must match FN_
{
...
void _fs3_my_get_dbndx(SG_context * pCtx, my_instance_data* pData, SG_uint32 iDagNum, SG_bool bQueryOnly, SG_dbndx** ppResult)
{
    SG_pathname* pPath = NULL;
    SG_dbndx* pndx = NULL;
...
void _fs3_my_get_treendx(SG_context * pCtx, my_instance_data* pData, SG_uint32 iDagNum, SG_bool bQueryOnly, SG_treendx** ppResult)
{
    SG_pathname* pPath = NULL;
...
void sg_repo__fs3__find_dagnodes_by_prefix(SG_context * pCtx, SG_repo * pRepo, SG_uint32 iDagNum, const char* psz_hid_prefix, SG_rbtree** pprb)
{
	my_instance_data * pData = NULL;
	pData = (my_instance_data *)pRepo->p_vtable_instance_data;
    ...
    )
{
    ...
    SG_RETRY_THINGIE(
        sg_blob_fs3__list_blobs(pCtx, pData->psql, blob_encoding, b_sort_descending, b_sort_by_len_encoded, limit, offset, ppvh)
        );
    ...
    )
{
    ...
    SG_ERR_CHECK(  sg_blob_fs3__store_blob__begin(pCtx, (my_tx_data*)pTx, psz_objectid, blob_encoding, psz_hid_vcdiff_reference, lenFull, lenEncoded, psz_hid_known, &pbh)  );
    ...
    )
{
    ...
    SG_ERR_CHECK(  sg_blob_fs3__fetch_blob__begin(
                pCtx,
                pData,
                psz_hid_blob,
                b_convert_to_full,
	...
	SG_pathname* pFragballPath)
{
	...
	// Add all blobs to fragball
	{
		...
		{
			pszHid = (const char*)sqlite3_column_text(pStmt, 0);
			SG_ERR_CHECK(  sg_blob_fs3__fetch_blob__begin(pCtx, pData, pszHid, SG_FALSE, &psz_objectid, &encoding, &pszHidVcdiffRef, &lenEncoded, &lenFull, SG_FALSE, &pbh)  );
			...
			next_offset = pbh->offset + lenEncoded;
...
void sg_repo__fs3__get_blob_stats(
    SG_context * pCtx,
    SG_repo * pRepo,
    SG_uint32* p_count_blobs_full,
    SG_uint32* p_count_blobs_alwaysfull,
    ...
    )
{
    ...
    SG_RETRY_THINGIE(
        sg_blob_fs3__get_blob_stats(pCtx, pData->psql,
            p_count_blobs_full,
    ...
    )
{
    ...
    SG_ERR_THROW_RETURN(  SG_ERR_REPO_FEATURE_NOT_SUPPORTED  );
}

void sg_repo__fs3__change_blob_encoding(
    SG_context * pCtx,
    SG_repo * pRepo,
	SG_repo_tx_handle* ptx,
    const char* psz_hid_blob,
    ...
    )
{
    ...
    SG_UNUSED(blob_encoding_desired);
    SG_UNUSED(psz_hid_vcdiff_reference_desired);
    SG_UNUSED(p_blob_encoding_new);
    SG_UNUSED(ppsz_hid_vcdiff_reference);
    SG_UNUSED(p_len_encoded);
    SG_UNUSED(p_len_full);
...
static void sg_fs3__nullfree_tx_data(SG_context* pCtx, my_tx_data** pptx)
{
    if (!pptx || !*pptx)
    {
        return;
	...
	SG_ERR_IGNORE(  sg_fs3__close_file_handles(pCtx, *pptx)  );
    SG_RBTREE_NULLFREE(pCtx, (*pptx)->prb_file_handles);
	SG_ERR_IGNORE(  sg_fs3__remove_lock_files(pCtx, *pptx)  );
    SG_RBTREE_NULLFREE(pCtx, (*pptx)->prb_append_locks);
	SG_VECTOR_NULLFREE_WITH_ASSOC(pCtx, (*pptx)->pvec_blobs, sg_fs3__free_pbi);
    ...
    )
{
    ...
    SG_ERR_CHECK(  SG_RBTREE__ALLOC(pCtx, &ptx->prb_file_handles)  );
    ...
    *pptx = ptx;
    ...
    return;
fail:
    SG_ERR_IGNORE(  sg_fs3__nullfree_tx_data(pCtx, &ptx)  );
}
	...
	)
{
    ...
    SG_ERR_CHECK(  sg_fs3__begin_tx(pCtx, pData, &ptx)  );
...
// this is a helper function called only from sg_fs3__commit_tx
static void sg_fs3__insert__prepare(SG_context * pCtx, sqlite3* psql, sqlite3_stmt** pp)
{
    sqlite3_stmt* pStmt = NULL;

    SG_ERR_CHECK(  sg_sqlite__prepare(pCtx, psql,&pStmt,
                                      "INSERT INTO directory (hid, filenumber, offset, encoding, len_encoded, len_full, hid_vcdiff, objectid) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")  );
...
static void sg_fs3__get_dbndx__cb(
        SG_context* pCtx,
        void * pVoidData,
        SG_uint32 iDagNum,
        SG_dbndx** ppndx
        )
{
    ...
    SG_ERR_CHECK_RETURN(  _fs3_my_get_dbndx(pCtx, pData, iDagNum, SG_FALSE, ppndx)  );
}
...
static void sg_fs3__cache_blob_info__pbi(
	SG_context * pCtx,
	my_instance_data* pData,
    struct pending_blob_info* pbi
    )
{
    SG_strpool* pPool = NULL;
    struct my_blob_info* pmybi = NULL;
    SG_bool b_already = SG_FALSE;

    SG_NULLARGCHECK_RETURN(pData);
    SG_NULLARGCHECK_RETURN(pbi);

    SG_ERR_CHECK(  SG_rbtree__find(pCtx, pData->prb_blob_info, pbi->sz_hid_blob_final, &b_already, (void**) &pmybi)  );
    if (b_already)
    {
        // TODO verify?
    }
    ...
    {
    SG_ERR_CHECK(  SG_rbtree__get_strpool(pCtx, pData->prb_blob_info, &pPool)  );
    SG_ERR_CHECK(  SG_strpool__add__len(pCtx, pPool, sizeof(struct my_blob_info), (const char**) &pmybi)  );
    pmybi->filenumber = pbi->filenumber;
    pmybi->offset = pbi->offset;
    pmybi->blob_encoding = pbi->blob_encoding_storing;
    pmybi->len_encoded = pbi->len_encoded_observed;
    pmybi->len_full = pbi->len_full_given;
    if (pbi->psz_hid_vcdiff_reference)
    {
        SG_ERR_CHECK(  SG_strpool__add__sz(pCtx, pPool, pbi->psz_hid_vcdiff_reference, &pmybi->psz_hid_vcdiff_reference)  );
    }
    if (pbi->psz_objectid)
    {
        SG_ERR_CHECK(  SG_strpool__add__sz(pCtx, pPool, pbi->psz_objectid, &pmybi->psz_objectid)  );
    }

    SG_ERR_CHECK(  SG_rbtree__add__with_assoc(pCtx, pData->prb_blob_info, pbi->sz_hid_blob_final, pmybi)  );
    }
	...
	)
{
    ...
    // TODO do we still need this transaction to be EXCLUSIVE ?
    ...
    SG_ERR_CHECK(  sg_fs3__insert__prepare(pCtx, pData->psql, &pStmt)  );
    for (i=0; i<count; i++)
    {
        struct pending_blob_info* pbi = NULL;
        ...
        SG_ERR_CHECK(  sg_sqlite__bind_text(pCtx, pStmt,8,pbi->psz_objectid)  );
        ...
        SG_ERR_CHECK(  sg_fs3__cache_blob_info__pbi( pCtx, pData, pbi)  );
    }
	SG_ERR_CHECK(  sg_sqlite__nullfinalize(pCtx, &pStmt)  );
    ...
    /* remove the lock files here.  */
	SG_ERR_CHECK(  sg_fs3__close_file_handles(pCtx, ptx)  );
	SG_ERR_CHECK(  sg_fs3__remove_lock_files(pCtx, ptx)  );
    ...
    if (prb_new_dagnodes)
    {
        SG_ERR_CHECK(  sg_repo__update_ndx(pCtx, pData->pRepo, prb_new_dagnodes,
                    sg_fs3__get_dbndx__cb, pData,
                    sg_fs3__get_treendx__cb, pData)  );
    }

fail:
    SG_ERR_IGNORE(  sg_fs3__nullfree_tx_data(pCtx, (my_tx_data**) pptx)  );
    SG_RBTREE_ITERATOR_NULLFREE(pCtx, pit);
    SG_RBTREE_NULLFREE(pCtx, prb_missing);
	...
	)
{
	...
	if (pData->pBlobStoreHandle)
	{
        SG_ERR_IGNORE(  sg_blob_fs3__store_blob__abort(pCtx, &pData->pBlobStoreHandle)  );
	}
	...
	)
{
    ...
    SG_ERR_CHECK(  _fs3_my_get_dbndx(pCtx, pData, iDagNum, SG_TRUE, &pndx)  );
...
void sg_repo__fs3__qresult__count(
	SG_context* pCtx,
    SG_repo* pRepo,
    SG_repo_qresult* pqr,
    SG_uint32* piCount
...
void sg_repo__fs3__qresult__get__multiple(
	SG_context* pCtx,
    SG_repo* pRepo,
    SG_repo_qresult* pqr,
...
void sg_repo__fs3__qresult__get__one(
	SG_context* pCtx,
    SG_repo* pRepo,
    SG_repo_qresult* pqr,
    SG_vhash** ppvh
...
void sg_repo__fs3__qresult__done(
	SG_context* pCtx,
    SG_repo* pRepo,
    SG_repo_qresult** ppqr
    )
...
void sg_repo__fs3__dbndx__lookup_audits(
	SG_context* pCtx,
    SG_repo* pRepo, 
    SG_uint32 iDagNum,
    const char* psz_csid,
...
void sg_repo__fs3__fetch_blob__info(
    SG_context* pCtx,
	SG_repo * pRepo,
    const char* psz_hid_blob,
...
void sg_repo__fs3__obliterate_blob(
    SG_context* pCtx,
	SG_repo * pRepo,
    const char* psz_hid_blob
    )
...
void sg_repo__fs3__treendx__get_path_in_dagnode(
        SG_context* pCtx,
        SG_repo* pRepo,
        SG_uint32 iDagNum,
...
void sg_repo__fs3__treendx__get_all_paths(
        SG_context* pCtx,
        SG_repo* pRepo,
        SG_uint32 iDagNum,
        const char* psz_gid,
...
void sg_repo__fs3__dbndx__query_across_states(
	SG_context* pCtx,
    SG_repo* pRepo,
    SG_uint32 iDagNum,
	const SG_varray* pcrit,
...
void sg_repo__fs3__check_integrity(
    SG_context* pCtx,
    SG_repo * pRepo,
    SG_uint16 cmd,
    SG_uint32 iDagNum,
