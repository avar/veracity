...
#include <sg.h>
#define SG_USE_SGGLUE

#include "sgbrings/js/jsstddef.h"
#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <locale.h>
#include "sgbrings/js/jstypes.h"
#include "sgbrings/js/jsarena.h"
#include "sgbrings/js/jsutil.h"
#include "sgbrings/js/jsprf.h"
#include "sgbrings/js/jsapi.h"
#include "sgbrings/js/jsarray.h"
#include "sgbrings/js/jsatom.h"
#include "sgbrings/js/jscntxt.h"
#include "sgbrings/js/jsdbgapi.h"
#include "sgbrings/js/jsemit.h"
#include "sgbrings/js/jsfun.h"
#include "sgbrings/js/jsgc.h"
#include "sgbrings/js/jslock.h"
#include "sgbrings/js/jsnum.h"
#include "sgbrings/js/jsobj.h"
#include "sgbrings/js/jsparse.h"
#include "sgbrings/js/jsscope.h"
#include "sgbrings/js/jsscript.h"
...
#define MSG_DEF(name, number, count, exception, format) \
    name = number,
#include "sgbrings/js/jsshell.msg"
#undef MSG_DEF
    JSShellErr_Limit
#undef MSGDEF
} JSShellErrNum;
...
Process(JSContext *cx, JSObject *obj, char *filename, JSBool forceTTY)
{
    ...
    do {
        ...
        *bufp = '\0';
        ...
        startline = lineno;
        do {
            if (!GetLine(cx, bufp, file, startline == lineno ? "vscript> " : "")) {
                hitEOF = JS_TRUE;
                break;
            }
            bufp += strlen(bufp);
...
main(int argc, char **argv, char **envp)
{
    ...
    * so we need to unbuffer then to get a reasonable prompt          */
...
#ifdef SG_USE_JSGLUE
    JS_SetContextCallback(rt, SG_jsglue__context_callback);
#else
    ...
    // We install the pCtx into the CX using the cx-private-data field.
    // This allows any of our methods/properties that we expose to
    // JavaScript (and that only get a CX pointer) to fetch our pCtx for
    // use with sglib routines.  It also means that we'll get a full
    // sg-style stacktrace when an error occurs.
    //
    // We ASSUME that the lifetime of the pCtx and CX are the same (or
    // rather that the pCtx lives longer than the CX) so that we don't
    // have to worry about un-setting the cx-private-data field when the
    // pCtx gets destroyed.
    //
    // We also assume that it is proper to do this on the CX and not the RT.
    // because the CX is (can be) per-thread.
    //
    // Also, we set the cx-private-data explicitly ourselves rather than
    // let SG_jsglue__set_sg_context() do it because at some point if we
    // want be multi-threaded, we'll need to set a unique per-thread-pCtx
    // on each per-thread-CX.

    SG_jsglue__set_sg_context(pCtx, cx);

    // register our sglib-access methods and properties.

    SG_jsglue__install_scripting_api(pCtx, cx, glob);
    if (SG_context__has_err(pCtx))
    {
        // TODO consider dumping stacktrace from pCtx when the init fails.
        return 1;
    }
    SG_zing_jsglue__install_scripting_api(pCtx, cx, glob);
    if (SG_context__has_err(pCtx))
    {
        // TODO consider dumping stacktrace from pCtx when the init fails.
        return 1;
    ...
    */
    ...
    if ((result != 0)  &&  (SG_context__has_err(pCtx)))
    {
        // TODO dump stacktrace from pCtx...
    }

    SG_jsglue__set_sg_context(NULL, cx);
